...
class MainActivity : AppCompatActivity() {
    ...
    private fun startAnimation2() {
        ...
        val animatorSet = AnimatorSet()
        animatorSet.play(nightSkyAnimator).before(heightAnimator).before(sunsetSkyAnimator)
        animatorSet.start()
    }

    private fun startAnimationScale() {
        val sunYStart = binding.sun.top.toFloat()
        val sunScaleY = binding.sky.scaleY
        val sunScaleX = binding.sky.scaleX

        val heightAnimator = ObjectAnimator
            .ofFloat(binding.sun, "scaleX", sunScaleX, 1.5f)
            .setDuration(1000)
        heightAnimator.interpolator = AccelerateInterpolator()

        val heightAnimator2 = ObjectAnimator
            .ofFloat(binding.sun, "scaleX", 1.5f, sunScaleX)
            .setDuration(1000)
        heightAnimator2.interpolator = AccelerateInterpolator()
        heightAnimator2.startDelay = 1000

        val animatorSet = AnimatorSet()
        animatorSet.play(heightAnimator).with(heightAnimator2)
        animatorSet.start()
    }
}
